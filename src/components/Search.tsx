            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                <div className="flex flex-col items-center mt-12 w-full max-w-2xl mx-auto">
                    ...
                    <div className="flex flex-col md:flex-row w-full max-w-145 mt-12">
                        <div className="relative md:flex-4 w-full" onClick={(e) => e.stopPropagation()}>
                            <img src={search} alt="search" className="absolute left-4 top-1/2 -translate-y-1/2 w-auto h-4 opacity-50 pointer-events-none" />
                            <input {...register("search", {onChange: () => dispatch(toggleSearchDropdown(true))})} type="text" placeholder="Search for a place..." className="bg-neutral-800 rounded-lg w-full h-12 px-10 pr-4 focus:outline-none focus:ring-2 focus:ring-neutral-400 focus:ring-offset-2 focus:ring-offset-neutral-900 transition-all" />
                            {dropdown &&
